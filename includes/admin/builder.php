...
class WP_School_Calendar_Builder {
    ...
    public function duplicate_calendar() {
        if ( isset( $_REQUEST['page'] ) && 'wpsc-builder' === $_REQUEST['page'] ) {
            $sendback = remove_query_arg( array( '_wp_http_referer', '_wpnonce', 'wpsc_duplicate', 'page' ), wp_get_referer() );
            
            if ( ! empty( $_GET['wpsc_duplicate'] ) ) {
                $calendar_id = intval( $_GET['wpsc_duplicate'] );
    ...
    public function admin_page() {
        ...
        if ( ! empty( $_GET['wpsc_edit'] ) ) {
            $calendar = wp_parse_args( wpsc_get_calendar( intval( $_REQUEST['wpsc_edit'] ) ), $calendar );
        }
    ...
    public function ajax_save_calendar() {
        ...
        if ( empty( $calendar_id ) ) {
            ...
            $result['replace'] = array( 
                ...
                'url'         => add_query_arg( 'wpsc_edit', $calendar_id, admin_url( 'edit.php?post_type=school_calendar&page=wpsc-builder' ) ) 
            );
        } else {
