    ...
    public function row_actions( $actions, $post ) {
        if ( in_array( $post->post_type, array( 'school_calendar', 'important_date' ) ) ) {
            ...
            if ( 'school_calendar' === $post->post_type ) {
                $edit_link      = add_query_arg( 'wpsc_edit', $post->ID, admin_url( 'edit.php?post_type=school_calendar&page=wpsc-builder' ) );
                $duplicate_link = wp_nonce_url( add_query_arg( 'wpsc_duplicate', $post->ID, admin_url( 'edit.php?post_type=school_calendar&page=wpsc-builder' ) ), 'calendar_duplicate-' . $post->ID );
                ...
                $actions = array(
                    ...
                    'wpsc_duplicate' => sprintf( '<a href="%1$s">%2$s</a>', esc_url( $duplicate_link ), esc_html( __( 'Duplicate', 'wp-school-calendar' ) ) ),
